...
export interface SleepScore {
  ...
  consistency_score: number; // 0-100 based on bedtime variance over last 7 days
  quality_score: number;     // 0-100 based on self-reported quality
  recovery_index: number;    // 0-100 combination score
  sleep_debt_minutes: number; // cumulative deficit over 7 days (target 480 min/night)
  recommendation: string;    // personalized tip
...
function getDurationScore(durationMinutes: number): number {
  if (durationMinutes >= 420 && durationMinutes <= 540) return 100;
  if ((durationMinutes >= 360 && durationMinutes < 420) || (durationMinutes > 540 && durationMinutes <= 600)) return 70;
  return 40;
}

function getConsistencyScore(records: SleepRecord[]): number {
  if (records.length < 2) return 100; // Not enough data, assume good consistency
  ...
  if (varianceMinutes < 30) return 100;
  if (varianceMinutes < 60) return 70;
  if (varianceMinutes < 90) return 50;
  return 30;
}

function getQualityScore(quality: SleepRecord['quality']): number {
  switch (quality) {
...
export function generateEnhancedRecommendations(
  ...
  qualityScore: number,
  sleepDebt: number,
  records: { date: string; bedtime: string; wake_time: string; quality: string; duration_minutes: number }[],
): SleepRecommendation[] {
  const all: SleepRecommendation[] = [];
  ...
  if (durationScore < 60) {
    all.push({
      ...
      category: 'duration',
      icon: '⏰',
      title: 'Duration Needs Improvement',
  ...
  if (qualityScore < 60) {
    all.push({
      ...
      category: 'quality',
      icon: '🛌',
      title: 'Sleep Quality Attention',
      tip: 'Your self-reported sleep quality suggests restless nights. Focus on creating a wind-down routine: dim lights, no screens 60 minutes before bed, and keep your room cool (65-68F / 18-20C).',
      priority: 'high',
  ...
  const priorityOrder: Record<SleepRecommendation['priority'], number> = {
    critical: 0,
    high: 1,
    medium: 2,
    low: 3,
...
export function generateContextualTip(
  ...
  if (!hasRecords) return 'Start logging your sleep to get personalized tips.';
  if (currentHour >= 20 && currentHour <= 23) {
    return 'Evening is here - time to wind down. Put away screens, dim the lights, and try reading or meditation to prepare for restful sleep.';
  }
  if (currentHour >= 6 && currentHour < 12) {
    return "Good morning! Log last night's sleep to track your patterns and get better insights.";
  }
  ...
  return "It's late - if you're having trouble sleeping, try deep breathing or a calming podcast.";
}

export function calculateSleepScore(records: SleepRecord[], referenceDate: string): SleepScore {
